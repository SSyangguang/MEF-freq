...
def read_M3FD(root_path):
    ...
    # extract every ir/vis directory path
    path_ir = list(filter(lambda path: path.find('ir') >= 0, path))
    path_vis = list(filter(lambda path: path.find('vi') >= 0, path))
    # ir image path list
    ir_img = []
    for path in path_ir:
        ir_img.extend(glob.glob(os.path.join(path, '*.png')))
    # vis image path list
    vis_img = []
    for path in path_vis:
        vis_img.extend(glob.glob(os.path.join(path, '*.png')))

    return ir_img, vis_img


def read_MEF(root_path):
    # root_path = 'E:\project\data\MEF_revise\Dataset_Part1'
    # extract every over/under directory root path
    path = []
    for root, dirs, files in os.walk(root_path, topdown=False):
        path.append(root)

    # over image path list
    img_over = []
    for p in path:
        img_over.extend(glob.glob(os.path.join(p, 'over.JPG')))
    # under image path list
    img_under = []
    for p in path:
        img_under.extend(glob.glob(os.path.join(p, 'under.JPG')))

    # return absolute path of images
    return img_over, img_under
